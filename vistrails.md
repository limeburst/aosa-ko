...
이 코드는 옛 GetItemsFromDirect(1.6까지의 버전들)를 사용하는 작업 흐름을 Directory 모듈을 사용하도록 업그레이드합니다. 옛 모듈의 dir 포트를 value로, itemlist 포트를 itemList로 매핑합니다.
...
위 문서를 pdflatex를 사용해서 컴파일 됬을 때, \vistrails 명령은 VisTrails 서버에 id 119를 가진 작업 흐름을 실행하도록 XML-RPC 요청을 보내는 파이썬 스크립트를 실행하며, 작업 흐름의 결과를 받아 지정된 레이아웃 옵션(width=0.9\linewidth)으로 링크 걸린 LaTeX \includegraphics 명령을 생성하여 PDF 문서에 포함시킵니다.
