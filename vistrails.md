...
VisTrails addresses important usability issues that have hampered a wider adoption of workflow and visualization systems. To cater to a broader set of users, including many who do not have programming expertise, it provides a series of operations and user interfaces that simplify workflow design and use [[FSC+06](http://aosabook.org/en/bib1.html#bib:freire:vistrails)], including the ability to create and refine workflows by analogy, to query workflows by example, and to suggest workflow completions as users interactively construct their workflows using a recommendation system [[SVK+07](http://aosabook.org/en/bib1.html#bib:scheidegger:analogy)]. We have also developed a new framework that allows the creation of custom applications that can be more easily deployed to (non-expert) end users.
...
Initial versions of versions of VisTrails were written in Java and C++ [[BCC+05](http://aosabook.org/en/bib1.html#bib:bavoil:vistrails)]. The C++ version was distributed to a few early adopters, whose feedback was instrumental in shaping our requirements for the system.
...
출처를 지원하는 시스템의 핵심 구성 요소는 데이터의 직렬화와 저장입니다. VisTrails는 처음에 데이터를 단순히 내부 객체(e.g., 버전 트리, 모듈)에 정의된 `fromXML`과 `toXML` 메서드를 통해 XML로 저장하였습니다. 이런 객체들의 스키마가 변하는 것을 지원하기 위해, 이 메서드들은 스키마 버전 간의 번역도 직렬화하였습니다. 프로젝트가 발전하며, 사용자층이 성장하였고, 우리는 관계형 저장소 등 다른 직렬화 방식을 지원할 필요가 생겼습니다. 또, 스키마 객체가 변하면서 스키마 버전 관리, 버전 간의 번역, 개체 간 관계의 지원 같은 공통적인 데이터 관리 문제를 개선해야 했으며, 이를 위해 데이터베이스(db) 층을 추가하였습니다.
...
모든 업그레이드는 업그레이드 전후의 실행들을 차별하고 비교할 수 있도록, 버전 트리에 새로운 버전을 등록합니다. 업그레이드가 작업 흐름의 동작을 바꿀 수도 있기 때문에, 이를 출처 정보로서 추적해야 합니다. 이전 버전의 VisTrails에서는 트리 안의 모든 버전을 업그레이드 해야 할수도 있으며, 어지러움을 줄이기 위해 사용자가 탐색한 적이 있는 버전들만 업그레이드 합니다. 또, 사용자가 작업 흐름이 편집되거나 실행될 때 까지 업그레이드의 영속을 미룰 수 있도록 설정할 수 있게 합니다. 사용자가 해당 버전을 열람만 한다면, 업그레이드를 영속시킬 필요가 없습니다.
...
재현성은 과학적 방법의 토대이지만, 계산적 실험들을 포함하는 최근의 출판물들은 결과물을 재현하거나 일반화하는데 필요한 충분한 정보를 제공하는 데 실패합니다. 최근 재현 가능한 결과의 출판에 대한 관심이 다시 높아지고 있습니다. 이러한 관습 확산의 가장 큰 장벽은 결과물을 재현하고 검증하기 위해 필요한 모든 구성 요소(e.g., 데이터, 코드, 매개 변수 설정)를 포함한 꾸러미를 만들기가 힘들다는 점입니다.
...
VisTrails는 자세한 출처를 추적하고, 위에 설명된 다양한 기능들을 통해 VisTrails 내에서 수행된 계산적 실험들의 과정을 단순화시킵니다. 하지만, 문서들을 연결하고 출처 정보를 공유하기 위해서는 체계가 필요합니다.
...
VisTrails 패키지를 개발하였습니다. 우리가 개발한 LaTeX 패키지를 통해 사용자들은 VisTrails 작업 흐름에 연결되는 그림들을 삽입할 수 있습니다. 다음 LaTeX 코드는 작업 흐름 결과를 포함하는 그림을 생성합니다:
...
위 문서를 pdflatex를 사용해서 컴파일 됬을 때, `\vistrails` 명령은 VisTrails 서버에 `id 119`를 가진 작업 흐름을 실행하도록 XML-RPC 요청을 보내는 파이썬 스크립트를 실행하며, 작업 흐름의 결과를 받아 지정된 레이아웃 옵션(`width=0.9\linewidth`)으로 링크 걸린 LaTeX `\includegraphics` 명령을 생성하여 PDF 문서에 포함시킵니다.
...
VisTrails의 결과물을 웹 페이지, 위키, 워드 문서, 파워포인트 프레젠테이션에 포함시키는 것 역시 가능합니다. 마이크로소프트 파워포인트와 VisTrails간의 연결은 컴포넌트 오브젝트 모델(COM)과 객체 연결 삽입(OLE) 인터페이스를 통해 이루어집니다. 객체가 파워포인트와 상호 작용하기 위해서는 최소한 COM의 `IOleObject`, `IDataObject`, 그리고 `IPersistentStorage` 인터페이스가 구현되어야 합니다. COM 인터페이스를 구현하는 데에 Qt의 `QAxAggregated` 추상화 클래스를 사용하기 때문에, OLE 오브젝트를 만드는 데 필요한 `IDataObject`와 `IPersistentStorage`는 Qt에 의해 자동으로 관리되므로, 우리는 `IOleObject` 인터페이스만 구현하면 됩ㄴ디ㅏ. 이 인터페이스의 가장 중요한 호출은 DoVerb입니다. VisTrails가 오브젝트 활성화 등 파워포인트에서의 특정 동작에 반응할 수 있도록 합니다. VisTrails의 구현은 VisTrails 오브젝트가 활성화되었을 때 VisTrails을 열어, 사용자들이 삽입하고 싶은 파이프라인과 상호 작용 하거나 선택할 수 있게 합니다. 사용자가 VisTrails를 종료하면 파이프라인 결과가 파워포인트에 보여집니다. 파이프라인 정보는 OLE 오브젝트와 함께 저장됩니다.
...
사용자들이 결과물을 연관된 출처 정보와 함께 자유롭게 공유할 수 있도록 crowdLabs를 만들었습니다. crowdLabs는 소셜 웹사이트입니다. crowdLabs는 VisTrails와 접목되어 있습니다. 사용자가 VisTrails에서 기인한 데이터를 공유하고 싶다면, VisTrails에서 바로 crowdLabs 서버에 접속하여 정보를 업로드 할 수 있습니다. 해당 정보가 업로드되면, 사용자들은 웹브라우저를 통해 작업 흐름과 상호 작용 하거나 실행시킬 수 있습니다. 이 작업 흐름들은 crowdLabs를 구동하는 VisTrails 서버에 의해 실행됩니다. VisTrails가 재현 가능한 발행물을 만드는 자세한 방법에 대해 알고 싶다면, http://www.vistrails.org를 보세요.
...
운이 좋게도, 데이터 탐색과 시각화 시스템을 만드는 것에 대해 생각하도 있었을 2004년도에는 이것이 이렇게 도전적이거나 지금까지 오는 데 오래 걸릴 줄 상상조차 하지 못했습니다. 만약 그걸 알았다면 시작조차 하지 않았을 것입니다.
...
초기에는, 새로운 기능들을 빠르게 프로토타이핑하고 선택된 일부 사용자들에게 보여주는 전략이 잘 작동했습니다. 초기에 이 사용자들에게 받은 피드백과 격려가 프로젝트를 진행시키는 데 큰 도움이 되었습니다. 사용자 피드백 없이는 VisTrails를 설계하기 불가능했을 것입니다. VisTrails의 한 측면을 강조할 수 있다면, VisTrails 대부분의 기능들은 사용자 피드백의 직접적인 영향으로 설계되었다는 점입니다. 하지만, 사용자들이 자주 요구하는 게 가장 최선의 해결책이 아니라는 것을 알 필요가 있습니다. 사용자들의 요구를 잘 맞춰 주는게 사용자들이 원하는 게 아니라는 것입니다. 우리는 VisTrails에 들어가는 기능들이 유용하고 시스템에 잘 접목되도록 몇 번이고 설계하고 재설계하였습니다.
...
우리의 사용자 중심의 접근을 감안하면, 우리가 개발한 모든 기능들이 많이 사용될 것이라고 생각할 수도 있습니다. 안타깝게도, 그렇지는 않습니다. 종종 그 이유가 되는 것은, 다른 도구에서는 찾아볼 수 없는 기능들이 매우 특이하다는 점입니다. 예를 들어, 유사와 버전 트리조차 대부분의 사용자들에게는 생소한 개념이며, 익숙해지는 데 오래 걸립니다. 다른 중요한 문제는 문서, 또는 문서의 부족입니다. 다른 오픈 소스 프로젝트들과 마찬가지로, 우리는 새로운 기능을 만드는 것을 이미 존재하는 기능을 문서화하는 것보다 잘 합니다. 문서화에 있어서의 뒤쳐짐은 유용한 기능들이 충분히 이용되지 않게 할 뿐만이 아니라, 메일링 리스트에 질문이 많이 올라오게 하기도 합니다.
...
모든 개발자들에게 감사의 말을 전하고 싶습니다: Erik Anderson, Louis Bavoil, Clifton Brooks, Jason Callahan, Steve Callahan, Lorena Carlo, Lauro Lins, Tommy Ellkvist, Phillip Mates, Daniel Rees, 그리고 Nathan Smith에게. 프로젝트의 비전을 만드는 데 도움을 준 Antonio Baptista, VisTrails를 개선, 특히 출처 정보가 풍부한 발행 기능을 연구, 개발, 그리고 릴리즈하기 위한 대부분의 자극을 준 Matthias Troyer에게 특별한 감사의 말을 전하고 싶습니다. VisTrails의 개발과 연구는 허가 IIS 1050422, IIS-0905385, IIS 0844572, ATM-0835821, IIS-0844546, IIS-0746500, CNS-0751152, IIS-0713637, OCE-0424602, IIS-0534628, CNS-0514485, IIS-0513692, CNS-0524096, CCF-0401498, OISE-0405402, CCF-0528201, CNS-0551724, Department of Energy SciDAC (VACET과 SDM 센터들), 그리고 IBM Faculty Awards 아래 National Science Foundation의 지원이 있었습니다.
